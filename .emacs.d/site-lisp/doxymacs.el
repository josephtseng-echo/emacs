...
\\{doxymacs-mode-map}"
  ...
  (when doxymacs-mode
    (when (boundp 'filladapt-token-table)
      ...
      (let ((bullet-regexp "[@\\]\\(t?param\\(?:\\s-*\\[\\(?:in\\|out\\|in,out\\)\\]\\)?\\s-+\\sw+\\|return\\)"))
  ...
  (list
   ...
   (list
    (concat "/\\*\\(?:\\*\\|!\\)+[ \t]*"
            ...
            "\\(?:\\(\\.\\)\\|"
                 "\\(?:[ \t]*\n[ \t]*\\)-#?\\|"
                 "\\(?:[ \t]*\n[ \t]*\\)?[\\@]\\(?:overload\\|name\\|li\\|ingroup\\|internal\\|par\\|param\\|return\\|warning\\|note\\|todo\\|bug\\|verbatim\\)\\|"
                 "\\(?:[ \t]*\n[ \t]*\\)<!--+ *!+ *--+>\\|"
                 "[ \t]*\\*+/\\)")
   ...
   (list
    ...
    (concat "\\([@\\\\]\\(brief\\|details\\|\\(end\\)?code\\|sa"
            "\\|note\\|\\(end\\)?verbatim\\|return\\|arg\\|fn"
            "\\|hideinitializer\\|showinitializer\\|remarks?"
            "\\|MC_[[:alnum:]]+"
            ;; FIXME
   ...
   (list
    (concat "\\([@\\\\]\\(t?param\\(?:\\s-*\\[\\(?:in\\|out\\|in,out\\)\\]\\)?"
   ...
   (list
    "\\([@\\\\]e\\(m\\)?\\)\\s-+\\([^ \t\n]+\\)"
    '(1 hi-semi-unimportant prepend)
    '(3 (quote bold) prepend)) ; replaced italic by bold
